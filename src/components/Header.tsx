...
export function Header({ set, done, total }: { set: string; done: number; total: number }) {
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
        <div className="flex items-center gap-2.5">
          <span className="rounded-sm border border-border px-1.5 py-px font-mono text-[10px] tracking-wider text-muted-foreground">
            {set}
          </span>
          <h1 className="text-sm font-medium">Cheat Sheet</h1>
        </div>
